...
utilitiesWith = pkgs: self: super: let
 ...
 library = haskellLib // (with haskellLib;
  { skipTests       = dontCheck
...
*/
aliasedDerivationTransformers = self: rec
  ...
  ; goldLinker          = gold;
  ; gold                = bool self.linkWithGold
                               id 
  
  ; deadCodeElimination = dce;
  ; dce                 = bool self.enableDeadCodeElimination
                               self.disableDeadCodeElimination 
  
  ; dwarfDebugging      = dwarf; 
  ; dwarf               = bool self.enableDWARFDebugging
  ...
  # ; packunused          = checkUnusedPackages;
